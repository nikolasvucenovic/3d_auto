...
The first pinned planner payload set is now registered in `offline_bundle/catalog.json`: llama.cpp `b10809`, its CUDA 12.4 runtime DLLs, and Qwen3-Coder 30B-A3B Instruct Q5_K_M. `modules/download.mjs` performs resumable single-stream acquisition, `modules/download-ranges.mjs` accelerates range-capable large files while retaining chunks, and `modules/verify-catalog.mjs` performs a read-only size and SHA-256 audit.
