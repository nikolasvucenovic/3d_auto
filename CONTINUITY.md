...
- The deterministic small and medium benchmarks pass. The large baseline deliberately exposes a remaining semantic-reference limitation around “the hero rises.”
...
1. Run the local-model small, medium, and large benchmarks and inspect plans plus previews.
2. Tune only the compact planner prompt, output limits, or validation repair if results fail; retain every run for comparison.
3. Select and pin an existing local Blender MCP implementation only after the evaluation in `docs/BLENDER_MCP_EVALUATION.md` is reproduced. Prefer reusing typed/read-only tools while keeping unrestricted Python disabled.
4. Add parent IDs and section diffs to ordinary UI jobs using the benchmark patch mechanism.
5. Benchmark offline behavior with networking disabled before calling the planner bundle transferable.
